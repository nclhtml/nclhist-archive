...
// Helper component to draw horizontal branching arrows between columns
function BranchingArrows({ leftCount, rightCount }) {
  ...
  return (
    <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ minWidth: '20px' }}>
      <defs>
...
export default function Practice2Combined({ savedData, onSave, readOnly, isAdmin, currentPart = 1, adminCommand }) {
  ...
  // Reset answers and lock when changing parts
  useEffect(() => {
  ...
  useEffect(() => {
    if (savedData) {
      ...
      if (savedData.part3Steps) setPart3Steps(savedData.part3Steps);
    } else {
      setMapPlacements({});
  ...
  // --- PART 3: BRANCHING DRAG HANDLER ---
  const handlePart3DragEnd = (e, info, optionId) => {
    if (effectiveReadOnly) return;
    const stepZones = document.querySelectorAll('.part3-step-zone');
    const insertZones = document.querySelectorAll('.part3-insert-zone');
    
    let droppedStepIndex = null;
    let droppedInsertIndex = null;

    const clientX = e.changedTouches ? e.changedTouches[0].clientX : e.clientX;
    const clientY = e.changedTouches ? e.changedTouches[0].clientY : e.clientY;

    stepZones.forEach((zone) => {
      const rect = zone.getBoundingClientRect();
      if (clientX >= rect.left && clientX <= rect.right && clientY >= rect.top && clientY <= rect.bottom) {
        droppedStepIndex = parseInt(zone.dataset.step, 10);
      }
    });
    ...
    insertZones.forEach((zone) => {
      const rect = zone.getBoundingClientRect();
      if (clientX >= rect.left && clientX <= rect.right && clientY >= rect.top && clientY <= rect.bottom) {
        droppedInsertIndex = parseInt(zone.dataset.insert, 10);
      }
    });

    if (droppedStepIndex === null && droppedInsertIndex === null) return;

    setPart3Steps((prev) => {
      let newSteps = prev.map(step => [...step]);
      
      for (let i = 0; i < newSteps.length; i++) {
        newSteps[i] = newSteps[i].filter(id => id !== optionId);
      }

      if (droppedStepIndex !== null) {
        newSteps[droppedStepIndex].push(optionId);
      } else if (droppedInsertIndex !== null) {
        newSteps.splice(droppedInsertIndex, 0, [optionId]);
      }
  ...
  const placedPart3Options = part3Steps.flat();
  ...
  return (
    <div className="w-full max-w-7xl mx-auto bg-slate-50 p-4 md:p-6 rounded-lg font-sans flex flex-col gap-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-slate-200 sticky top-0 z-10">
        <div>
      ...
      {partNum === 1 && (
        <section className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200 mb-6 min-h-[80px] flex flex-wrap gap-3 items-center">
            ...
            {availableCountries.map((country) => (
              <motion.div
                ...
                className={`px-4 py-2 bg-white border-2 border-slate-300 rounded shadow-sm font-bold text-slate-700 touch-none ${!effectiveReadOnly ? 'cursor-grab' : 'cursor-not-allowed opacity-70'}`}
              >
                {country}
              </motion.div>
            ))}
          ...
          <div className="relative w-full max-w-4xl mx-auto border-4 border-slate-200 rounded-xl overflow-hidden bg-blue-50" ref={mapRef}>
            <img 
              src="https://i.imgur.com/TBqjLvZ.png" 
              alt="Europe Map WWI" 
                ...
                >
                  {placedCountry && (
                    <motion.div
                      ...
                      style={{ touchAction: 'none' }}
                      className={`w-full h-full flex items-center justify-center font-bold text-sm touch-none ${!effectiveReadOnly ? 'cursor-grab' : 'cursor-default'}
                  ...
                  {showAnswers && !isCorrect && (
                    <div className="absolute -bottom-8 bg-white border border-green-500 text-green-700 text-xs font-bold px-2 py-1 rounded shadow-lg z-10 whitespace-nowrap">
                      正確: {zone.correct}
                    </div>
                  )}
                </div>
            ...
            <div className="flex flex-col gap-2">
              {availableSeqOptions.map((opt) => (
                <motion.div
                  ...
                  style={{ touchAction: 'none' }}
                  className={`p-3 bg-white border-2 border-slate-300 rounded shadow-sm font-bold text-slate-700 touch-none ${!effectiveReadOnly ? 'cursor-grab' : 'cursor-not-allowed opacity-70'}`}
                >
                  {opt.id}. {opt.text}
                </motion.div>
              ))}
                ...
                <React.Fragment key={index}>
                  <div
                    ...
                    className={`seq-drop-zone relative w-full md:w-1/4 h-32 border-2 border-dashed rounded-lg flex items-center justify-center bg-white transition-colors p-2
                      ...
                      ${showAnswers && placedId && isCorrect ? '!border-green-500 !bg-green-50' : ''}
                      ${showAnswers && (!placedId || !isCorrect) ? '!border-red-500 !bg-red-50' : ''}
                    `}
                  >
                    ...
                    {placedOpt && (
                      <motion.div
                        ...
                        style={{ touchAction: 'none' }}
                        className={`w-full h-full mt-4 flex items-center justify-center text-center font-bold text-sm touch-none p-1 ${!effectiveReadOnly ? 'cursor-grab' : 'cursor-default'}
                          ${showAnswers && isCorrect ? 'text-green-700' : ''}
                          ${showAnswers && !isCorrect ? 'text-red-700' : 'text-slate-800'}
                        `}
                      >
                    ...
                    {showAnswers && !isCorrect && (
                      <div className="absolute -bottom-12 left-0 right-0 bg-white border border-green-500 text-green-700 text-xs font-bold px-2 py-1 rounded shadow-lg z-10 text-center">
                        正確: {correctOpt.id}
                      </div>
                    )}
                  </div>
      ...
      {partNum === 3 && (
        <section className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col gap-2">
              {availablePart3Options.map((opt) => (
                <motion.div
                  ...
                  drag={!effectiveReadOnly}
                  dragSnapToOrigin
                  onDragEnd={(e, info) => handlePart3DragEnd(e, info, opt.id)}
                  whileDrag={{ scale: 1.02, zIndex: 50, cursor: 'grabbing' }}
                  style={{ touchAction: 'none' }}
                  className={`p-3 bg-blue-50 border-2 border-blue-300 rounded shadow-sm font-bold text-blue-900 touch-none ${!effectiveReadOnly ? 'cursor-grab' : 'cursor-not-allowed opacity-70'}`}
                >
                  {opt.text}
                </motion.div>
              ))}
          ...
          <div className="flex flex-row items-stretch justify-between pb-8 pt-4 px-2 min-h-[300px] w-full bg-white rounded-xl border-2 border-slate-200 shadow-inner overflow-hidden">
            <div data-insert="0" className="part3-insert-zone relative flex-1 max-w-[40px] group">
               <div className="absolute inset-y-4 left-1/2 -translate-x-1/2 w-6 border-2 border-dashed border-transparent group-hover:border-blue-400 group-hover:bg-blue-50/50 rounded-lg transition-colors z-10 flex items-center justify-center">
                  <span className="opacity-0 group-hover:opacity-100 text-blue-400 font-bold">+</span>
               </div>
            </div>

            {part3Steps.map((stepItems, index) => (
              <React.Fragment key={`step-${index}`}>
                <div 
                  data-step={index} 
                  className="part3-step-zone flex flex-col justify-center gap-3 w-1/4 max-w-[160px] p-1 md:p-2 rounded-xl border-2 border-dashed border-transparent hover:border-blue-300 hover:bg-blue-50/30 transition-colors"
                >
                  {stepItems.map(itemId => {
                    ...
                    return (
                      <motion.div
                        key={itemId}
                        drag={!effectiveReadOnly && !!isExtra}
                        dragSnapToOrigin
                        onDragEnd={(e, info) => handlePart3DragEnd(e, info, itemId)}
                        whileDrag={{ scale: 1.05, zIndex: 50 }}
                        style={{ touchAction: 'none' }}
                        className={`w-full p-2 rounded-lg shadow-sm font-bold text-xs md:text-sm text-center flex items-center justify-center aspect-square
                          ${isBase ? 'bg-white border-2 border-slate-300 text-slate-800' : 'bg-blue-50 border-2 border-blue-400 text-blue-900 cursor-grab'}
                        `}
                      >
                ...
                <div data-insert={index + 1} className="part3-insert-zone relative flex-1 max-w-[50px] group flex items-center justify-center">
                   {index < part3Steps.length - 1 && (
                      <BranchingArrows leftCount={stepItems.length} rightCount={part3Steps[index + 1].length} />
                   )}
                   <div className="absolute inset-y-4 left-1/2 -translate-x-1/2 w-8 border-2 border-dashed border-transparent group-hover:border-blue-400 group-hover:bg-blue-50/50 rounded-lg transition-colors z-10 flex items-center justify-center">
